...
const LIGHT_PAPER = "#f5f5f0"; // ivory-2
...
<body class="antialiased min-h-screen relative overflow-hidden m-0 p-0">

    <!-- Visualization Background -->
    <div class="fixed inset-0 z-0 overflow-hidden bg-transparent">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function resize() {
                ...
                // Regenerate paths so they distribute evenly across the real height
                paths.length = 0;
...
const GATEWAY_FLOW_DEFINITION: EffectDefinition = {
  ...
  supportsMode: true,
  background: (mode) => (mode === "light" ? "transparent" : "transparent"), // Changed to transparent so hero bg shows
  ...
  patch(source, { size, density, mode }) {
    ...
    if (mode === "light") {
      next = next
        .replace(
          "ctx.strokeStyle = 'rgba(255, 255, 255, 0.35)';",
          "ctx.strokeStyle = 'rgba(212, 163, 42, 0.8)';", // honey-5 with high opacity
        )
        .replace(
          "ctx.fillStyle = `rgba(255, 255, 255, 0.7)`;",
          "ctx.fillStyle = `rgba(212, 163, 42, 1.0)`;", // honey-5 solid
        );
    }
    return next;
  },
...
[data-threeui-role="background"] { position: absolute !important; inset: 0 !important; width: 100% !important; height: 100% !important; max-width: none !important; max-height: none !important; z-index: 0 !important; opacity: 1 !important; pointer-events: none !important; }
